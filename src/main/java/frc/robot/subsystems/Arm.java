...
public class Arm extends SubsystemBase {
    ...
    public final Trigger armLengthStall = new Trigger(() -> lengthMotor.getOutputCurrent() >= LENGTH_CURRENT_STALL);

    public final Trigger armFullyClosedTrigger = new Trigger(() -> lengthEncoder.getPosition() <= MINIMAL_LENGTH_METERS);
    ...
    public Arm() {
        ...
        lengthMotor.setSmartCurrentLimit(70);
...
//    setDefaultCommand(setAngleSpeed(0));
    ...
    public Command resetLengthCommand() {
        return new ParallelRaceGroup(
                ...
                new WaitCommand(0.25).andThen(new WaitUntilCommand(armLengthStall)));
    }
    ...
    public Command manualLengthCommand(DoubleSupplier lengthSpeed) {
        return new RunCommand(() -> lengthMotor.set(lengthSpeed.getAsDouble() / 2), this);
    }
...
//            moveToAngleCommand(baseAngle.rotateBy(Rotation2d.fromDegrees(magnitude))).withTimeout(3)
    ...
    public void periodic() {
        if (armLengthStall.getAsBoolean()) lengthEncoder.setPosition(MINIMAL_LENGTH_METERS);
        angleRelativeEncoder.setPosition(getArmAngle());

        SmartDashboard.putNumber("lengthCurrent", lengthMotor.getOutputCurrent());
        SmartDashboard.putNumber("length", lengthEncoder.getPosition() * 100);
    }
